...
import 'package:codon/features/videos/screens/video_modules_screen.dart';
...
class VideoChaptersScreen extends StatelessWidget {
  final SubSubjectModel subSubject;
  const VideoChaptersScreen({super.key, required this.subSubject});
  ...
  Widget _buildChapterCard(dynamic chapter) {
    return Container(
      ...
      child: Stack(
        children: [
          InkWell(
            onTap: () => Get.to(() => VideoModulesScreen(chapter: chapter)),
            borderRadius: BorderRadius.circular(15),
            child: Padding(
